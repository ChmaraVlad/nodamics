...
import {GetFootprintQueryResponse} from "../interface/api/footprintQuery/getFootprintQuery";
    ...
    endpoints: (builder) => ({
        ...
        getDiagramById: builder.query<{
            ...
            queryFn: async (diagramId: string) => {
                const socket = await getSocketAsync({namespace: '/diagram'});
        ...
        getFootprint: builder.query<{
            content?: GetFootprintQueryResponse
        }, {
            diagramId: string | undefined
        }>({
            queryFn: async ({diagramId}) => {
                const socket = await getSocketAsync({namespace: '/footprint'});
                socket.emit(EFootprintSocketEventEmit.JoinFootprintRoom, diagramId);
                socket.emit(EFootprintSocketEventEmit.RequestFootprint, diagramId);
                return {
                    data: {}
                }
            },
            async onCacheEntryAdded(
                ...
                try {
                    ...
                    socket.on(EFootprintSocketEventOn.UPDATE_FOOTPRINT, (content: any) => {
                        updateCachedData((draft) => {
                    ...
                    await cacheEntryRemoved;
                    socket.off('connect');
                    socket.off(EFootprintSocketEventEmit.JoinFootprintRoom);
                    socket.off(EFootprintSocketEventEmit.RequestFootprint);
                } catch {
                    // if cacheEntryRemoved resolved before cacheDataLoaded,
                    // cacheDataLoaded throws
                }

import {GraphBaseNode, GraphDataNode, GraphDatasetDatafieldNode, GraphLoopNode} from "../GraphNodes";
...
export class GraphNodeManager {
    ...
    updateParentChildrenList({parentId}: {parentId: string}){
        const parentNode = this.findById({nodeId: parentId})
        if(parentNode && parentNode instanceof GraphLoopNode){
            parentNode.updateChildrenNodesList()
        }
    }
}

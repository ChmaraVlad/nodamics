...
export class RunManager {
    ...
    async executeNode(chainItem: IChainItem, nodeToExecute: NodeExecutionManager, options: { invoke: boolean }) {
        ...
        if (target instanceof GraphInvokableNode) {
            ...
            if (isInvoke) {
                ...
                target.invokeStep({
                    targetMode: chainItem.edge?.targetMode
                })
                ...
                // TODO fix it later
                // add outgoing nodes of accumulative loop to execute
                ...
                if (target instanceof GraphLoopNode) {
                    ...
                    if (target instanceof GraphMicroLoopNode && target.data.isAccumulative || target instanceof GraphWhileLoopNode) {
                        ...
                        nodeToExecute.addNodesToCurrent(target, innerNodes)
                    }
                }
            ...
            if (target instanceof GraphStartNode) {
                nodeToExecute.addNodesToCurrent(target, outgoingConnected)
            } else {
                ...
                const dataNodes = outgoingConnected.filter(item => item.target instanceof GraphDataNode && item.edge instanceof GraphDataEdge)
                if (target instanceof GraphMicroLoopNode) {

                    if (!target.isLoopActive && target.isAccumulative && !isChildOfAccumLoop) {
                        // add outgoing nodes to execute from accum loop
                        nodeToExecute.addNodesToExecute(noDataNodes, true)
                    } else if(!target.isLoopActive && target.isAccumulative && isChildOfAccumLoop) {
                        nodeToExecute.addNodesToCurrent(target, noDataNodes)
                    }
                    ...
                    nodeToExecute.addNodesToCurrent(target, dataNodes)
                }
                const endChainItem = children.endChainItem

                if (endChainItem && endChainItem.target instanceof GraphMicroLoopNode) {
                    // this.clearChildrenOfmLoop(endChainItem.target, nodeToExecute)

                    if (!endChainItem.target.isLoopActive) {
                        nodeToExecute.addNodesToExecute([endChainItem])
                    }

                    if (!endChainItem.target.isAccumulative && endChainItem.target.isLoopActive) {
                            nodeToExecute.addNodesToExecute([endChainItem])
                    }


                    if(endChainItem.target.isLoopActive && endChainItem.target.isAccumulative){
                        nodeToExecute.addNodesToCurrent(endChainItem.target,[endChainItem])
                    }

                    if (!endChainItem.target.isLoopActive && endChainItem.edge?.isMeetCondition) {

                            const children = this.getChainChildren(endChainItem)
                        endChainItem.target.children.forEach(child => {
                            if (child instanceof GraphMicroLoopNode) {
                                child.resetLoopStep()
                            }
                        })
                        nodeToExecute.addNodesToExecute(children.outgoingConnected, true)
                    }
    ...
    clearChildrenOfmLoop(target: GraphLoopNode, nodeToExecute: NodeExecutionManager) {
        const innerNodeIds = target.children
